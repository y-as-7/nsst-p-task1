...
const Page = () => {
  ...
  function getSum(e) {
    e.preventDefault();
    let i =
      Number(y1.current.value) * Number(z2.current.value) -
      Number(z1.current.value) * Number(y2.current.value);
    let k =-
      (Number(x1.current.value) * Number(z2.current.value) -
      Number(x2.current.value) * Number(z1.current.value))
    let j =
      Number(x1.current.value) * Number(y2.current.value) -
      Number(x2.current.value) * Number(y1.current.value);
    setMul([i,k,j]);
    clearValues();
  }
  return (
    <div className="mt-28 w-[90%] mx-auto">
      ...
      <div className="mt-8 flex flex-col w-[90%] md:w-[600px] mx-auto">
        ...
        <form action="">
          ...
          <div className="">
            <Link className="mx-1" href="/laws/9">
        ...
        <div className="my-3 text-center">
          { mul ? (
            <div>
              ...
              <p className="text-red-700 text-2xl  ">V&#8407; =( <span className="text-gray-600">{mul[0]}</span> i&#8407; , <span className="text-gray-600">{mul[1]}</span> k&#8407; , <span className="text-gray-600">{mul[2]}</span> j&#8407; )</p>
            </div>
          ) : (
